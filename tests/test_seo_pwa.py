...
def seo_fixture(tmp_path, monkeypatch):
    ...
    db.execute("INSERT INTO movies(stable_id,title,slug,parsing_confidence,adult_content) VALUES ('adult','Adult Movie','adult-movie','high',1)")
    db.close()
    return app
...
def test_sitemap_and_robots_are_public_safe(tmp_path, monkeypatch):
    ...
    assert b"adult-movie" not in sitemap and b"/admin/" not in sitemap
